# Data and Agents Lab

Data and Agents Lab is a student-friendly training repository for learning through hands-on work in Jupyter notebooks.

It is designed for:
- high school students
- beginner interns
- mentors and coaches

The repository helps students practice:
- Python basics
- notebook skills
- pandas and tabular data
- APIs and JSON
- debugging and testing
- plotting and interpretation
- classic AI / data investigation
- agentic AI workflow thinking
- using AI as a helper, not autopilot

## Start Here

1. Read [12-how-to-use-the-practice-notebooks.md](./12-how-to-use-the-practice-notebooks.md)
2. Read [17-four-week-study-plan.md](./17-four-week-study-plan.md)
3. Open `notebooks/00-notebook-basics-practice.ipynb`
4. Start practicing

## Repository Map

Core student guides:
- `02-foundations-guide.md`
- `03-classic-ai-training-guide.md`
- `04-agentic-ai-training-guide.md`
- `05-using-ai-well.md`
- `16-using-free-ai-tools.md`
- `17-four-week-study-plan.md`

Support guides:
- `06-python-for-jupyter-notebook-challenges.md`
- `07-pandas-and-data-basics.md`
- `08-apis-json-and-live-data.md`
- `09-debugging-and-testing.md`
- `10-plotting-and-data-visualization.md`
- `11-getting-started-resources.md`
- `13-glossary.md`
- `14-progress-checklist.md`
- `15-mentor-guide.md`

Practice notebooks:
- `notebooks/00-notebook-basics-practice.ipynb`
- `notebooks/01-python-todo-practice.ipynb`
- `notebooks/02-pandas-data-practice.ipynb`
- `notebooks/03-api-and-json-practice.ipynb`
- `notebooks/04-ai-assisted-problem-solving-practice.ipynb`
- `notebooks/05-mini-classic-ai-challenge.ipynb`
- `notebooks/06-mini-agentic-ai-challenge.ipynb`
- `notebooks/07-debugging-practice.ipynb`
- `notebooks/08-plotting-and-interpretation.ipynb`
- `notebooks/09-agentic-workflow-practice.ipynb`
- `notebooks/10-multi-step-data-investigation.ipynb`

## Visual Guide

![Learning Path](./assets/learning-path-diagram.png)

## Notes

This repo is ready for guided student practice and pilot use.
Students do not need paid AI accounts for the core learning path.

- `18-how-to-open-and-run-notebooks-in-google-colab.md`
